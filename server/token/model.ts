...
export const tokenModel: TokenModel = {
  ...
  updateOneByContent: async (content, update) => {
    return await db.update(TokenTable).set(TokenTableUpdateSchema.parse(update)).where(eq(TokenTable.content, ContentSchema.parse(content).content)).returning().then(r => r[0])
  },
